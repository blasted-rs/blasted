...
pub fn get_line_ending(line: &RopeSlice) -> Option<LineEnding> {
  ...
  // First check the two-character case for CRLF, then check the
...
pub fn line_without_line_ending<'a>(
  slice: &'a RopeSlice,
  line_idx: usize,
) -> RopeSlice<'a> {
...
pub fn rope_end_without_line_ending(slice: &RopeSlice) -> usize {
  slice.len_chars()
    - get_line_ending(slice).map(|le| le.len_chars()).unwrap_or(0)
}
...
mod line_ending_tests {
  ...
  fn line_ending_autodetect() {
    ...
    assert_eq!(
      auto_detect_line_ending(&Rope::from_str(
        "a formfeed\u{000C} with a\u{000C} carriage return \
         linefeed\u{000D}\u{000A} and a linefeed\u{000A}"
      )),
      Some(LineEnding::Crlf)
    );
  }
